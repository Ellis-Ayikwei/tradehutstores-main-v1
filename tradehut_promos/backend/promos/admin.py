...
from .models import PromoCode, PromoRedemption, PromoAttempt, ReferralCode


@admin.register(PromoCode)
class PromoCodeAdmin(admin.ModelAdmin):
    list_display  = (
        "code", "name", "discount_badge", "target_type",
        "is_live_display", "usage_display", "min_order_value",
        "schedule_display", "revenue_influenced_display", "updated_at",
    )
    list_filter   = ("is_active", "discount_type", "target_type", "user_segment", "auto_apply", "stackable")
    search_fields = ("code", "name")
    readonly_fields = (
        "current_redemptions", "is_live_display",
        "revenue_influenced_display", "created_at", "updated_at",
    )
    ...
    fieldsets = (
        ("Identity", {
            "fields": ("code", "name", "description", "is_active", "auto_apply", "auto_apply_priority"),
        }),
        ("Discount Rule", {
            "fields": (
                "discount_type", "discount_value", "max_discount_amount",
                "buy_quantity", "get_quantity", "fixed_price",
            ...
            "description": (
                ...
                "BUY_X_GET_Y: fill buy_quantity and get_quantity. "
                "FIXED_PRICE: fill fixed_price."
            ),
        }),
        ("What It Applies To", {
            "fields": ("target_type", "products", "categories"),
            "description": "ENTIRE_ORDER applies to everything. PRODUCTS/CATEGORIES restrict eligibility.",
        }),
        ("Eligibility", {
            "fields": (
                "min_order_value", "min_items_count",
                "user_segment", "specific_users",
                "first_order_only", "stackable",
            ),
        }),
        ("Usage Limits", {
            "fields": ("max_redemptions", "current_redemptions", "max_redemptions_per_user"),
        }),
        ("Schedule", {
            "fields": ("starts_at", "ends_at", "is_live_display"),
        }),
        ("Analytics", {
            "fields": ("revenue_influenced_display",),
            "classes": ("collapse",),
        }),
        ("Timestamps", {
    ...
    def discount_badge(self, obj):
        ...
        labels = {
            ...
            DT.FIXED_PRICE:  (f"Fix GHS {obj.fixed_price}",      "#8b5cf6"),
        }
    ...
    def usage_display(self, obj):
        if obj.max_redemptions:
            ...
            return format_html(
                ...
                colour, obj.current_redemptions, obj.max_redemptions,
            )
    ...
    def export_codes_csv(self, request, queryset):
        ...
        writer.writerow(["Code", "Name", "Type", "Value", "Uses", "Max Uses", "Expires"])
        for promo in queryset:
            writer.writerow([
                promo.code, promo.name, promo.discount_type,
                promo.discount_value, promo.current_redemptions,
                promo.max_redemptions or "Unlimited",
                promo.ends_at.strftime("%Y-%m-%d") if promo.ends_at else "No expiry",
            ])
        return response

    @admin.action(description="⊕ Generate 10 bulk codes from selected template")
        ...
        for template in queryset:
            for _ in range(10):
                ...
                PromoCode.objects.create(
                    code=code, name=f"{template.name} (bulk)",
                    ...
                    is_active=False,  # require manual review before activating
                )
                created += 1
