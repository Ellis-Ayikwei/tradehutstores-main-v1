...
# Fallback serializers to avoid import issues
...
from .models import OTP, TrustedDevice, UserVerification
...
class RegisterAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Import custom exceptions early
        ...
        try:
            ...
            # Try validation - this might raise EmailAlreadyExistsException or PhoneNumberAlreadyExistsException
            try:
                ...
                # Catch custom exceptions raised during validation
            ...
            if is_valid:
                ...
                for field, error_list in serializer.errors.items():
                    if isinstance(error_list, list):
                        # Convert each error item to string if it's not already
                        formatted_errors[field] = [str(err) for err in error_list]
                    elif isinstance(error_list, dict):
                        # Nested errors (e.g., nested serializer)
                        formatted_errors[field] = error_list
                    else:
                        # Single error as string
                        formatted_errors[field] = [str(error_list)]

                # Get the first error message for a generic message
                first_field = (
                    list(formatted_errors.keys())[0] if formatted_errors else None
                )
                first_error = (
                    formatted_errors[first_field][0]
                    if first_field and formatted_errors[first_field]
            ...
            # Handle specific conflict exceptions (exceptions that weren't caught during validation)
            if isinstance(e, EmailAlreadyExistsException):
                return Response(
                    {
                        "message": "User with this email address already exists.",
...
class AdminLoginAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if getattr(user, "is_staff", None) != True:
                return Response(
                    {"detail": "Admins only: this endpoint is restricted."},
        ...
        try:
            ...
            try:
                ...
                if device_id and device_fp:
                    ...
                    # Check if device exists and fingerprint matches
                    if td and td.device_fingerprint_hash == fp_hash:
                        # Check if device is expired
                        if td.expires_at and td.expires_at <= now:
                            logger.warning(
                                f"Trusted device expired for user {user.id}, device_id {device_id}. Expires_at: {td.expires_at}, Now: {now}"
                            )
                            # Extend expiry if device is expired but fingerprint matches
                            td.expires_at = now + timedelta(
                                days=int(os.getenv("DEVICE_TRUST_EXPIRY_DAYS", "30"))
                            )
                            td.is_active = True
                            logger.info(
                                f"Extended expiry for trusted device {device_id} for user {user.id}"
                            )
                        ...
                        # Bind refresh to device by storing hash and extend expiry
                        td.refresh_token_hash = hashlib.sha256(
                            str(refresh).encode()
                        ).hexdigest()
                        td.last_used = now
                        # Extend expiry date on each successful login
                        if not td.expires_at or td.expires_at <= now:
                            td.expires_at = now + timedelta(
                                days=int(os.getenv("DEVICE_TRUST_EXPIRY_DAYS", "30"))
                            )
                        td.save(
                        ...
                        logger.info(
                            f"Trusted device recognized for user {user.id}, device_id {device_id}. Issuing tokens directly."
                        )
                        ...
                        # Device exists but fingerprint doesn't match
                        logger.warning(
                            f"Device fingerprint mismatch for user {user.id}, device_id {device_id}. "
                            f"Stored hash: {td.device_fingerprint_hash[:16]}..., Received hash: {fp_hash[:16]}..."
                        )
            except Exception as e:
                # Log the exception for debugging but fall back to OTP flow
                logger.error(
                    f"Error checking trusted device for user {user.id}: {str(e)}",
                    exc_info=True,
                )
...
class AdminViewUserOTPsView(APIView):
    """Admin endpoint to view current OTPs for a user"""
    ...
    def get(self, request, user_id=None):
        ...
        if not request.user.is_staff:
            return Response(
                {
                    "message": "Only staff members can view user OTPs.",
                    "error_code": "INSUFFICIENT_PERMISSIONS",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            ...
            # Get current active OTPs for the user
            current_time = timezone.now()
            active_otps = OTP.objects.filter(
                user=user, is_used=False, expires_at__gt=current_time
            ).order_by("-created_at")

            # Format OTP data
            otp_data = {}
            for otp in active_otps:
                otp_data[otp.otp_type] = {
            ...
            return Response(
                {
                    ...
                    "otps": otp_data,
                    "total_active_otps": len(otp_data),
            ...
            logger.error(f"Error in AdminViewUserOTPsView: {str(e)}")
            return Response(
                {
                    "message": "Failed to retrieve user OTPs.",
