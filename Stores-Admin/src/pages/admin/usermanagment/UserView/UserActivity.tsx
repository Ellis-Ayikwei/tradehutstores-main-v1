...
import { History, User, LogIn, LogOut, Key, Shield, FileText, CreditCard, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';
...
const UserActivity: React.FC<UserActivityProps> = ({ user }) => {
  ...
  const { 
    ...
    user?.id ? `/users/${user.id}/activity/` : null,
    fetcher,
    {
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden">
        ...
        <div className="p-6">
          <div className="text-center py-8">
            ...
            <p className="text-red-600 dark:text-red-400">Failed to load user activities</p>
            <button onClick={() => mutate()} className="btn btn-primary">Try Again</button>
          </div>
        </div>
      </div>
    );
