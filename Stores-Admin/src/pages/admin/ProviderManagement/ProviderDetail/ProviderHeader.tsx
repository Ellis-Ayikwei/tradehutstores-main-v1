import { ArrowLeft, Award, Ban, Building, Calendar, CheckCircle2, Clock, Edit, Globe, Loader2, Mail, MapPin, Phone, Shield, ShieldX, Star, Trash2, UserCheck, UserX } from 'lucide-react';
...
export const ProviderHeader: React.FC<ProviderHeaderProps> = ({ provider, onActivate, onSuspend, onVerify, onUnverify, onDelete, saveLoading }) => {
        ...
        <div className="bg-white dark:bg-gray-800  shadow-lg overflow-hidden">
            ...
            <div className="relative h-48 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                    ...
                    {provider.user.account_status === 'active' && (
                        <button
                            className="flex items-center justify-center w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 transition-all duration-200"
                            onClick={onSuspend}
                            title="Suspend Provider"
                        >
                            {saveLoading.suspend ? <Loader2 className="w-4 h-4 animate-spin" /> : <Ban className="w-4 h-4" />}
                            {saveLoading.suspend ? 'Suspending...' : 'Suspend'}
                        </button>
                    )}
                    <button
                        className="flex items-center justify-center w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 transition-all duration-200"
                    ...
                    >
                        {saveLoading.delete ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                        {saveLoading.delete ? 'Deleting...' : 'Delete'}
                    </button>
                </div>
            </div>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
                                ...
                                {provider.base_location && (
                                    <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                        ...
                                        <span className="truncate">
                                            {(() => {
                                                try {
                                                    if (typeof provider.base_location === 'string') {
                                                        const parsed = JSON.parse(provider.base_location);
                                                        return parsed.address || 'Location available';
                                                    }
                                                    return 'Location available';
                                                } catch (error) {
                                                    console.warn('Error parsing base_location:', error);
                                                    return 'Location available';
                                                }
                                            })()}
                                        </span>
                        ...
                        <div className="flex flex-col gap-3 lg:ml-8 mt-6 lg:mt-0">
                            ...
                            <div className="flex flex-col gap-2">
                                {provider.user.account_status !== 'active' && (
                                    <button
                                        className="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-xl flex items-center justify-center gap-2 transition-colors font-medium"
                                        onClick={onActivate}
                                    >
                                        {saveLoading.activate ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                                        {saveLoading.activate ? 'Activating...' : 'Activate'}
                                    </button>
                                )}

                                {provider.verification_status !== 'verified' && (
                                    <button
                                        className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl flex items-center justify-center gap-2 transition-colors font-medium"
                                        onClick={onVerify}
                                    >
                                        {saveLoading.verify ? <Loader2 className="w-4 h-4 animate-spin" /> : <Shield className="w-4 h-4" />}
                                        {saveLoading.verify ? 'Verifying...' : 'Verify'}
                                    </button>
                                )}
                                {provider.verification_status === 'verified' && (
                                    <button
                                        ...
                                        onClick={onUnverify}
                                    >
                                        {saveLoading.unverify ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShieldX className="w-4 h-4" />}
                                        {saveLoading.unverify ? 'Unverifying...' : 'Unverify'}
                                    </button>
                                )}
                            </div>
                        </div>
