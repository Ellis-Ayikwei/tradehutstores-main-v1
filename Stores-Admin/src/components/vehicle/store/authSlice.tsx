...
export const LoginUser = createAsyncThunk('auth/LoginUser', async ({ email, password, extra }: { email?: string; password: string; extra?: any }, { rejectWithValue }) => {
    ...
    try {
        ...
        // Update device fingerprint user tracking after successful login
        // This will check if user has changed and clear fingerprint if needed
        getOrCreateDeviceId('mv_device_id', user?.email || email || null, user?.username || user?.first_name || null);
...
export const MfaLoginUser = createAsyncThunk('auth/MfaLoginUser', async (
    ...
    const { device_id, device_name, fingerprint, device_info } = getDeviceInfo(email, null);
